...
def evaluate(embeds, ratio, idx_train, idx_val, idx_test, label, nb_classes, device, dataset, lr, wd):
    ...
    for _ in range(5):
        if idx_train is None:
            indices = torch.randperm(label_num).to(device)
            idx_train = indices[0:int(ratio * label_num / 100)]
            idx_val = indices[int(ratio * label_num / 100):int((ratio + (100 - ratio) / 2) * label_num / 100)]
            idx_test = indices[int((ratio + (100 - ratio) / 2) * label_num / 100):]
    ...
    print("Ratio: {} | Macro-F1_mean: {:.4f} var: {:.4f} | Micro-F1_mean: {:.4f} var: {:.4f} | auc {:.4f}"
            .format(ratio,
                    np.mean(macro_f1s),
                    np.std(macro_f1s),
                    np.mean(micro_f1s),
